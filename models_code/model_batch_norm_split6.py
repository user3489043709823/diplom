# убираю установленные веса и заменяю это на max pooling
import keras.utils
    ...
    for piece in pieces_names:
        if piece != 'KING':
            ...
            global_pooling_layers[('AVERAGE', color, piece)] = GlobalAveragePooling2D(name='AVERAGE' + color + piece) \
                (bitboards_input[color + ' ' + piece])
            global_pooling_layers[('MAX', color, piece)] = GlobalMaxPooling2D(name='AVERAGE' + color + piece) \
                (bitboards_input[color + ' ' + piece])
